...
async function loadGraphFromApi(): Promise<BeliefMemoryGraph | null> {
  const candidates = [
    "/api/memory/graph?namespace=default",
    "/api/graph?namespace=default",
    "/api/memory/context?task=dashboard&max_variables=100",
  ];
...
function sampleMemory(): BeliefMemoryGraph {
  const memory = normalizeMemory({
    ...
    memgraph_lab_url: memgraphLabUrl,
    nodes: [
      {
        id: "01",
        type: "Claim",
