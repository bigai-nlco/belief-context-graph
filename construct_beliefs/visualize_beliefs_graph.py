...
|                       |  Belief graph (SVG)               |
|                       |    nodes  = beliefs               |
|  Conversation         |    edges  = confirms/contradicts/ |
|  trajectory           |             extends relations     |
|  (with segment-       |    node label = #ID + confidence  |
|   aware evidence      +-----------------------------------+
|   highlighting)       |  Detail panel                     |
|                       |    (filled when a node or edge    |
|                       |     is clicked)                   |
+-----------------------+-----------------------------------+

Differences from visualize_beliefs_v3.py
----------------------------------------
1. **Segment-aware evidence search**. An excerpt from an `assistant_other`
   (final-answer) belief is searched ONLY within the assistant_other span of
   the host message, never inside its `<think>` or `<tool_call>` blocks.
2. **Graph view** of belief relations. Nodes are positioned by
   (trajectory_index, layer); curved edges show confirms / contradicts /
   extends. Clicking a node lights up its evidence and fills the detail panel
   with full belief content, confidence history, and excerpts. Clicking an
   edge shows the linker note plus the two beliefs it connects.
...
def segment_message(role: str, content: str) -> List[Dict[str, Any]]:
    """
    Return a list of segments for one message, ordered as they appear.
    Each segment is {index, type, start, end}.
    The start/end are absolute offsets in the message content (including the
    opening / closing tags for tag-wrapped segments — matching the segmenter
    in construct_beliefs/segment.py).
    """
    segs_raw: List[Tuple[int, int, str]] = []  # (start, end, type)
    if role == "system":
        return []
    if role == "assistant":
        for m in _THINK_RE.finditer(content):
            segs_raw.append((m.start(), m.end(), "think"))
        for m in _TOOL_CALL_RE.finditer(content):
            segs_raw.append((m.start(), m.end(), "tool_call"))
        occupied = [(s, e) for (s, e, _) in segs_raw]
        for (s, e) in _carve_outside(content, occupied):
            segs_raw.append((s, e, "assistant_other"))
    elif role in ("user", "tool", "function"):
        for m in _TOOL_RESPONSE_RE.finditer(content):
            segs_raw.append((m.start(), m.end(), "tool_response"))
        occupied = [(s, e) for (s, e, _) in segs_raw]
        for (s, e) in _carve_outside(content, occupied):
            t = "user_input" if role == "user" else "tool_response"
            segs_raw.append((s, e, t))
        if not segs_raw and role in ("tool", "function") and content.strip():
            segs_raw.append((0, len(content), "tool_response"))
    segs_raw.sort()
    out: List[Dict[str, Any]] = []
    seg_idx = 0
    for (s, e, t) in segs_raw:
        if not content[s:e].strip():
            continue
        out.append({"index": seg_idx, "type": t, "start": s, "end": e})
        seg_idx += 1
    return out


# =============================================================
# Excerpt -> original-text position resolution (segment-bounded)
# =============================================================
...
def find_positions_in_range(
    original: str, excerpt: str, range_start: int, range_end: int,
) -> List[Tuple[int, int]]:
    """Find every occurrence of `excerpt` inside `original[range_start:range_end]`."""
    if not excerpt or not excerpt.strip():
        return []
    sub = original[range_start:range_end]
    if not sub:
        return []
    norm_sub, idx_map = normalize_with_map(sub)
    norm_ex = re.sub(r'\s+', ' ', excerpt).strip()
    if not norm_ex:
        return []
    positions: List[Tuple[int, int]] = []
    start = 0
    while True:
        pos = norm_sub.find(norm_ex, start)
        if pos < 0:
            break
        local_start = idx_map[pos]
        norm_end = pos + len(norm_ex) - 1
        local_end = idx_map[norm_end] + 1 if norm_end < len(idx_map) else len(sub)
        positions.append((local_start + range_start, local_end + range_start))
        start = pos + 1
    return positions
...
ALLOWED_SOURCES = {'user_input', 'llm_reasoning', 'tool_result',
                   'historical_retrieval', 'tool_call', 'assistant_other'}

SOURCE_LABEL = {
    'user_input':           'user input',
    'llm_reasoning':        'llm reasoning',
    'tool_result':          'tool result',
    'historical_retrieval': 'historical retrieval',
    'tool_call':            'tool call',
    'assistant_other':      'final answer',
}
...
.stat-dot.user_input          { background: var(--src-user-fg); }
.stat-dot.llm_reasoning       { background: var(--src-llm-fg); }
.stat-dot.tool_result         { background: var(--src-tool-fg); }
.stat-dot.historical_retrieval{ background: var(--src-hist-fg); }
.stat-dot.tool_call           { background: var(--src-call-fg); }
.stat-dot.assistant_other     { background: var(--src-final-fg); }
...
.node.source-user_input           .node-pill { fill: var(--src-user-bg);   stroke: var(--src-user-fg); }
.node.source-user_input           .node-conf { fill: var(--src-user-fg); }
.node.source-user_input           .node-id   { fill: var(--src-user-fg); opacity: 0.8; }
.node.source-llm_reasoning        .node-pill { fill: var(--src-llm-bg);    stroke: var(--src-llm-fg); }
.node.source-llm_reasoning        .node-conf { fill: var(--src-llm-fg); }
.node.source-llm_reasoning        .node-id   { fill: var(--src-llm-fg); opacity: 0.8; }
.node.source-tool_result          .node-pill { fill: var(--src-tool-bg);   stroke: var(--src-tool-fg); }
.node.source-tool_result          .node-conf { fill: var(--src-tool-fg); }
.node.source-tool_result          .node-id   { fill: var(--src-tool-fg); opacity: 0.8; }
.node.source-historical_retrieval .node-pill { fill: var(--src-hist-bg);   stroke: var(--src-hist-fg); }
.node.source-historical_retrieval .node-conf { fill: var(--src-hist-fg); }
.node.source-historical_retrieval .node-id   { fill: var(--src-hist-fg); opacity: 0.8; }
.node.source-tool_call            .node-pill { fill: var(--src-call-bg);   stroke: var(--src-call-fg); }
.node.source-tool_call            .node-conf { fill: var(--src-call-fg); }
.node.source-tool_call            .node-id   { fill: var(--src-call-fg); opacity: 0.8; }
.node.source-assistant_other      .node-pill { fill: var(--src-final-bg);  stroke: var(--src-final-fg); }
.node.source-assistant_other      .node-conf { fill: var(--src-final-fg); }
.node.source-assistant_other      .node-id   { fill: var(--src-final-fg); opacity: 0.8; }
...
.badge.source-user_input          { color: var(--src-user-fg); background: var(--src-user-bg); }
.badge.source-llm_reasoning       { color: var(--src-llm-fg);  background: var(--src-llm-bg); }
.badge.source-tool_result         { color: var(--src-tool-fg); background: var(--src-tool-bg); }
.badge.source-historical_retrieval{ color: var(--src-hist-fg); background: var(--src-hist-bg); }
.badge.source-tool_call           { color: var(--src-call-fg); background: var(--src-call-bg); }
.badge.source-assistant_other     { color: var(--src-final-fg); background: var(--src-final-bg); }
...
function renderNodeDetail(beliefId) {
  ...
  const breadcrumb = (typeof src.trajectory_index === 'number' && src.trajectory_index >= 0)
    ? `traj[${src.trajectory_index}] · ${escapeHtml(src.role || '?')}`
        + (src.segment_type ? ` · ${escapeHtml(src.segment_type)} segment[${src.segment_index}]` : '')
...
def render_html(data: Dict[str, Any], src_path: str) -> str:
    ...
    # Precompute segment ranges per message (so excerpt search is segment-bounded).
    segs_by_msg: Dict[int, List[Dict[str, Any]]] = {}
    for i, m in enumerate(trajectory):
        segs_by_msg[i] = segment_message(m.get('role') or '', m.get('content') or '')

    # For each belief, compute the (start, end) ranges of its excerpts.
    # Group beliefs by trajectory_index for left-panel rendering.
    by_msg: Dict[int, List[Tuple[int, Dict[str, Any], List[Tuple[int, int]]]]] = {}
    for b in all_beliefs:
        bid = b['id']
        src = b.get('source') or {}
        ti = src.get('trajectory_index')
        seg_idx = src.get('segment_index')
        seg_type = src.get('segment_type')
        if not isinstance(ti, int) or not (0 <= ti < len(trajectory)):
            continue
        host_content = trajectory[ti].get('content', '') or ''
        host_segs = segs_by_msg.get(ti, [])

        # Find the originating segment range. Match on segment_index first, then by type.
        target_seg = None
        if isinstance(seg_idx, int):
            for s in host_segs:
                if s['index'] == seg_idx:
                    target_seg = s; break
        if target_seg is None and seg_type:
            for s in host_segs:
                if s['type'] == seg_type:
                    target_seg = s; break

        if target_seg is not None:
            r_start, r_end = target_seg['start'], target_seg['end']
        else:
            r_start, r_end = 0, len(host_content)

        excerpt_status: Dict[str, bool] = {}
        all_ranges: List[Tuple[int, int]] = []
        for ex in (b.get('supporting_excerpts') or []):
            if not isinstance(ex, str):
                excerpt_status[str(ex)] = False
                continue
            positions = find_positions_in_range(host_content, ex, r_start, r_end)
            if positions:
                excerpt_status[ex] = True
                all_ranges.extend(positions)
            else:
                excerpt_status[ex] = False
        # Stash status on the belief object for JS to render.
        b['_evidence_status'] = excerpt_status
        by_msg.setdefault(ti, []).append((bid, b, all_ranges))

    # Render left panel (trajectory with segment-aware highlights).
    msg_panels = []
    for i, m in enumerate(trajectory):
    ...
    # Stats counters
    SRC_ORDER = ('user_input', 'tool_call', 'llm_reasoning', 'tool_result',
                 'assistant_other', 'historical_retrieval')
