...
# =====================================================================
# Factor-note abstraction prompt
# =====================================================================

_FACTOR_INPUT_NODE_PLACEHOLDER = "<<<FACTOR_INPUT_NODE>>>"
_FACTOR_OUTPUT_NODE_PLACEHOLDER = "<<<FACTOR_OUTPUT_NODE>>>"
_FACTOR_RELATION_TYPE_PLACEHOLDER = "<<<FACTOR_RELATION_TYPE>>>"
_FACTOR_DIRECTION_PLACEHOLDER = "<<<FACTOR_DIRECTION>>>"
_FACTOR_RELATION_NOTE_PLACEHOLDER = "<<<FACTOR_RELATION_NOTE>>>"

_FACTOR_NOTE_PROMPT = """\
# Task
Generate a reusable Factor activation note for a belief computation graph.

A Relation is one concrete semantic edge. Its `note` explains why that specific edge exists.
A Factor is a reusable computation mechanism. Its `activation_condition["note"]` should describe the slot-level mechanism by which the input node affects the output node.

Use the concrete edge context below as evidence, but DO NOT concatenate the fields and DO NOT merely paraphrase the relation note.

## Concrete edge context
Input node text:
<<<FACTOR_INPUT_NODE>>>

Relation type and direction:
<<<FACTOR_RELATION_TYPE>>> — <<<FACTOR_DIRECTION>>>

Output node text:
<<<FACTOR_OUTPUT_NODE>>>

Relation note for the concrete edge:
<<<FACTOR_RELATION_NOTE>>>

## What to generate
Write ONE short natural-language sentence for `note` that captures the reusable influence mechanism.

The note should be more abstract than the concrete edge, but less abstract than a generic label.

Keep:
- the type of input signal/source/evidence/premise,
- the type of output belief/decision being influenced,
- the support/refute mechanism,
- the kind of claim or reasoning step when it matters.

Remove or generalize:
- concrete entity names, document names, people, places, product names, and organizations,
- exact numbers, dates, ids, license plates, URLs, and one-off facts,
- wording that only applies to this one edge.

Good examples:
- "An authoritative source reports exact statistics that corroborate a numerical claim."
- "A verified hoax classification refutes an earlier factual assertion."
- "A final answer depends on a prior evidence-matching conclusion."
- "A newer deadline statement replaces an earlier deadline belief."

Bad examples:
- "The IPID report confirms 217 torture cases and 3,661 assault cases."
- "Node 8 supports node 10."
- "Evidence supports the claim."
- "The relation depends on the source."

## Output JSON only
{
  "note": "<one reusable mechanism-level sentence>"
}
"""


def build_factor_note_prompt(
    *,
    input_node_text: str,
    output_node_text: str,
    relation_type: str,
    direction_text: str,
    relation_note: str,
) -> str:
    """Prompt used only for Factor-note abstraction.

    This deliberately does not change relation extraction or relation.note.
    """
    prompt = _FACTOR_NOTE_PROMPT
    prompt = prompt.replace(_FACTOR_INPUT_NODE_PLACEHOLDER, input_node_text or "")
    prompt = prompt.replace(_FACTOR_OUTPUT_NODE_PLACEHOLDER, output_node_text or "")
    prompt = prompt.replace(_FACTOR_RELATION_TYPE_PLACEHOLDER, relation_type or "")
    prompt = prompt.replace(_FACTOR_DIRECTION_PLACEHOLDER, direction_text or "")
    prompt = prompt.replace(_FACTOR_RELATION_NOTE_PLACEHOLDER, relation_note or "")
    return prompt
