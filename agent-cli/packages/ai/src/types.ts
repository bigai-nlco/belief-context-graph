...
import type { BcgMessagesOptions } from "./api/bcg-messages.ts";
...
export interface ApiOptionsMap {
	...
	"bcg-messages": BcgMessagesOptions;
}
...
export interface OpenAICompletionsCompat {
	...
	thinkingFormat?:
		...
		| "string-thinking"
		| "ant-ling";
	/** Kwargs to send as `chat_template_kwargs` when `thinkingFormat` is `chat-template`. Use `{ "$var": "thinking.enabled" }` or `{ "$var": "thinking.effort" }` for bcg-controlled thinking values. */
...
export interface Model<TApi extends Api> {
	...
	reasoning: boolean;
	/**
	 * Maps bcg thinking levels to provider/model-specific values.
