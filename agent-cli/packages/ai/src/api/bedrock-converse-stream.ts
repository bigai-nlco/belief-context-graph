...
function addCustomHeadersMiddleware(client: BedrockRuntimeClient, headers: Record<string, string>): void {
	...
	client.middlewareStack.add(middleware, { step: "build", name: "bcg-ai-custom-headers", priority: "low" });
}
 ...
 * Defaults to "short" and uses BCG_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("BCG_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}
