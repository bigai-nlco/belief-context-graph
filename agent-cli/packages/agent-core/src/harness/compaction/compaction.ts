...
export interface CompactionResult<T = unknown> {
	...
	summary: string;
	/** Entry id where retained history starts. Optional during BCG 2.0 transition. */
	firstKeptEntryId?: string;
	/** Estimated context tokens before compaction. */
	tokensBefore: number;
	/** Usage from the LLM call(s) that generated this summary, if available. */
	usage?: Usage;
	/** Retained recent messages stored directly on the compaction entry. Optional during BCG 2.0 transition. */
