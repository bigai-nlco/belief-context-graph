...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.BCG_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.BCG_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	constructor(terminal: Terminal, showHardwareCursor?: boolean, logDirectory?: string) {
		...
		this.logDirectory = logDirectory ?? process.env.BCG_CODING_AGENT_DIR ?? path.join(os.homedir(), ".bcg", "agent");
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.BCG_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(this.logDirectory, "bcg-debug.log");
		...
		// Configurable via setClearOnShrink() or BCG_CLEAR_ON_SHRINK=0 env var
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(this.logDirectory, "bcg-crash.log");
				const crashData = [
		...
		if (process.env.BCG_TUI_DEBUG === "1") {
