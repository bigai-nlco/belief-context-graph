import type { Api, Model, ProviderHeaders } from "@bigai-nlco/bcg-ai";
...
function getDefaultAttributionHeaders(
	...
	if (isOpenRouterModel(model)) {
		return {
			"HTTP-Referer": "https://github.com/bigai-nlco/belief-context-graph",
			"X-OpenRouter-Title": "BCG",
	...
	if (isNvidiaNimModel(model)) {
		return {
			"X-BILLING-INVOKE-ORIGIN": "BCG",
		};
	}

	if (isCloudflareModel(model)) {
		return {
			"User-Agent": "bcg-agent",
		};
	}
...
function getSessionHeaders(model: Model<Api>, sessionId: string | undefined): Record<string, string> | undefined {
	...
	return { "x-opencode-session": sessionId, "x-opencode-client": "bcg" };
}
