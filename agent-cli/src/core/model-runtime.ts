...
export function shouldIncludeBuiltinProvider(
	providerId: string,
	env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
	const usesCustomOpenAIEndpoint = Boolean(env.OPENAI_BASE_URL?.trim());
	return !(usesCustomOpenAIEndpoint && providerId === "openai");
}
...
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const providers = builtinProviderCatalog
			.builtinProviders()
			.filter((provider) => shouldIncludeBuiltinProvider(provider.id));
