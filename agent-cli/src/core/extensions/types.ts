...
import type {
	...
	ThinkingLevel,
	ToolExecutionMode,
} from "@bigai-nlco/bcg-agent-core";
import type {
	Api,
...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@bigai-nlco/bcg-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@bigai-nlco/bcg-agent";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown bcg and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what BCG loaded without re-discovering resources. */
...
export interface ExtensionAPI {
	 ...
	 * bcg.registerProvider("anthropic", {
	 *   baseUrl: "https://proxy.example.com"
	 * });
	 *
	 * @example
	 * // Register provider with OAuth support
	 * bcg.registerProvider("corporate-ai", {
	 ...
	 *
	 * @example
	 * bcg.unregisterProvider("my-proxy");
	 */
...
/** Configuration for registering a provider via bcg.registerProvider(). */
...
export interface ProviderModelConfig {
	...
	reasoning: boolean;
	/** Maps bcg thinking levels to provider/model-specific values; null marks a level unsupported. */
...
export type ExtensionFactory = (bcg: ExtensionAPI) => void | Promise<void>;
...
export interface ExtensionRuntimeState {
	...
	/** Native bcg-ai provider registrations queued during extension loading, processed when runner binds. */
...
/**
 * Action implementations for bcg.* API methods.
