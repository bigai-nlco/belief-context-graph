...
import { contentText } from "@bigai-nlco/bcg-ai";
import type {
	AssistantMessage,
	AuthResult,
	ImageContent,
...
import { createAllToolDefinitions, isSerperSearchConfigured } from "./tools/index.ts";
...
export class AgentSession {
	 ...
	 * - Handles extension commands (registered via bcg.registerCommand) immediately, even during streaming
	...
	async prompt(text: string, options?: PromptOptions): Promise<void> {
		...
		try {
			...
			// Extension commands manage their own LLM interaction via bcg.sendMessage()
	...
	private _buildRuntime(options: {
		...
		const defaultActiveToolNames = this._baseToolsOverride
			...
			: ["read", "bash", "edit", "write", ...(isSerperSearchConfigured() ? ["web_search"] : [])];
