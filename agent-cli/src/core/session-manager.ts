...
import { type ImageContent, type Message, type TextContent, type Usage, uuidv7 } from "@bigai-nlco/bcg-ai";
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, undefined/false if bcg-generated (backward compatible) */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	usage?: Usage;
	/** True if generated by an extension, false if bcg-generated */
	fromHook?: boolean;
}
 ...
 * Encodes cwd into a safe directory name under ~/.bcg/agent/sessions/.
 */
...
export class SessionManager {
	...
	private _setSessionFile(sessionFile: string, preloadedFileEntries?: FileEntry[]): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			// non-empty but did not parse as a bcg session, fail without modifying it.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid bcg session: ${explicitPath}`);
				}
				this.newSession();
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.bcg/agent/sessions/<encoded-cwd>/).
	 */
