...
// provider-agnostic and does not import bcg-ai/compat itself.
setDefaultStreamFn(streamSimple);

export interface CreateAgentSessionOptions {
	/** Working directory for project-local discovery. Default: process.cwd() */
	cwd?: string;
	/** Global config directory. Default: ~/.bcg/agent */
	agentDir?: string;
	 ...
	 *
	 * When omitted, bcg enables the default built-in tools (read, bash, edit, write)
 ...
 *
 * // With explicit model
 * import { getModel } from '@bigai-nlco/bcg-ai';
 * const { session } = await createAgentSession({
...
export async function createAgentSession(options: CreateAgentSessionOptions = {}): Promise<CreateAgentSessionResult> {
	...
	if (isSerperSearchConfigured()) {
		defaultActiveToolNames.push("web_search");
	}
	...
	const getBcgContextManager = (): BcgContextManager | undefined => {
		...
		bcgContextManager ??= new BcgContextManager({
			...contextManagementSettings.bcg,
	...
	agent = new Agent({
		...
		transformContext: async (messages, signal) => {
			...
			return runner.emitContext(contextMessages);
		},
