...
import type { Transport } from "@bigai-nlco/bcg-ai";
...
export interface Settings {
	...
	httpProxy?: string; // Proxy URL applied as HTTP_PROXY and HTTPS_PROXY for BCG-managed HTTP clients
...
export class SettingsManager {
	...
	getContextManagementSettings(): ResolvedContextManagementSettings {
		const settings = this.settings.contextManagement;
		const configuredRecentTurns = settings?.bcg?.recentTurns;
		const recentTurns =
			typeof configuredRecentTurns === "number" && Number.isFinite(configuredRecentTurns)
				? Math.max(-1, Math.trunc(configuredRecentTurns))
				: 2;
		const configuredTimeoutMs = settings?.bcg?.timeoutMs;
		const timeoutMs =
	...
	setContextManagementProvider(provider: ContextManagementProvider): void {
		if (!this.globalSettings.contextManagement) {
			this.globalSettings.contextManagement = {};
		}
		this.globalSettings.contextManagement.provider = provider;
		this.markModified("contextManagement", "provider");
		this.save();
	}
	...
	getClearOnShrink(): boolean {
		...
		return process.env.BCG_CLEAR_ON_SHRINK === "1";
	}
	...
	getShowHardwareCursor(): boolean {
		return this.settings.showHardwareCursor ?? process.env.BCG_HARDWARE_CURSOR === "1";
	}
