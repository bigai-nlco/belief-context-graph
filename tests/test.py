...
from construct_beliefs.llm import (                                 # noqa: E402
    LocalEmbeddingClient, load_config, load_embedding_config, make_embedder)
from construct_beliefs.loaders import iter_items, load_input_file  # noqa: E402
from construct_beliefs.pipeline import run_item                    # noqa: E402
...
def fake_call_model(client, model, prompt, temperature=0.0, max_tokens=None,
    ...
    if "## Candidate beliefs" in prompt:
        ...
        return json.dumps({"merge_groups": [{
            "ids": ids, "canonical_belief": canonical,
    ...
    if "## Full belief list" in prompt:
        ...
        groups = [{"ids": sorted(v), "canonical_belief": k,
    ...
    if "## Full belief graph" in prompt:
        ...
        if len(ids) >= 2:
            rels.append({"from_id": ids[-1], "to_id": ids[0],
                         "type": "confirms", "note": "fake: later confirms earliest"})
        if len(ids) >= 3:
            rels.append({"from_id": ids[-1], "to_id": ids[1],
                         "type": "contradicts", "note": "fake: later contradicts second"})
    ...
    if "## Current turn sentences" in prompt:
        ...
        first = sents[0][1]
        b0 = {"tmp_id": "n0", "belief": f"S-claim: {first[:48]}", "stance": "asserted",
        ...
        if "silver Honda Civic" in block:
            b0 = {"tmp_id": "n0", "belief": "The user drives a silver Honda Civic.",
                  "stance": "asserted", "supporting_sentence_indices": [int(sents[0][0])],
                  "event_time": None, "time_text": None}
        beliefs.append(b0)
        if "March 15th" in block:
            beliefs.append({"tmp_id": "n1",
                            "belief": "The user's car was first serviced on March 15th.",
                            "stance": "asserted",
                            "supporting_sentence_indices": [int(s[0]) for s in sents],
                            "event_time": "2023-03-15", "time_text": "March 15th"})
        elif len(sents) >= 2:
            beliefs.append({"tmp_id": "n1",
        ...
        if existing:
            fwd.append({"from": max(existing), "to": "n0", "type": "informs",
                        "note": "fake: prior node informs first new"})
        if len(beliefs) >= 2:
            fwd.append({"from": "n0", "to": "n1", "type": "informs",
    ...
    if "## Current turn content" in prompt:
        ...
        if m and len(beliefs) >= 1:
            beliefs.append({"tmp_id": "n1",
                            ...
                            "stance": "asserted", "supporting_excerpts": ["828 meters"],
                            "event_time": None, "time_text": None})
        if existing and beliefs:
            fwd.append({"from": max(existing), "to": "n0", "type": "informs",
                        "note": "fake: prior informs new"})
        if len(beliefs) >= 2:
            fwd.append({"from": "n0", "to": "n1", "type": "informs", "note": "fake: chain"})
        return json.dumps({"beliefs": beliefs, "forward_relations": fwd})
...
def check_common(res, out_dir, tag):
    ...
    assert (out_dir / "logs" / "merge_final.json").exists(), f"[{tag}] missing merge_final.json"
    assert (out_dir / "logs" / "merge_final.log").exists(), f"[{tag}] missing merge_final.log"
    print(f"  [{tag}] {len(beliefs)} beliefs, "
...
def main() -> None:
    ...
    llm.USAGE.reset()
    opts = StreamOptions(evidence_mode="sentence", use_clustering=True,
                         cluster_threshold=0.4, cluster_min_sentences=2,
                         merge_strategy="embedding", merge_threshold=0.86)
    out_c = base / "conversation" / item["item_id"]
    res_c = run_item(item, client=None, model="fake-chat", out_dir=out_c,
                     options=opts, embedder=FakeEmbedder())
    check_common(res_c, out_c, "conversation")

    # whole-sentence evidence used
    assert any(ev.get("via") == "split_sentence"
               for b in res_c["all_beliefs"] for ev in b["evidence"]), \
    ...
    civic = [b for b in res_c["all_beliefs"]
    ...
    for rec in res_c["merges"]:
        for a in rec["absorbed_ids"]:
            assert a not in {b["id"] for b in res_c["all_beliefs"]}, f"absorbed id {a} active"
    ...
    assert any(h.get("step") == "contradicts"
               for b in res_c["all_beliefs"] for h in b["confidence_history"]), \
    ...
    res_r = run_item(item_r, client=None, model="fake-chat", out_dir=out_r, options=opts_r)
    check_common(res_r, out_r, "research")

    src_types = {(b.get("source") or {}).get("type") for b in res_r["all_beliefs"]}
    assert {"user", "assistant", "tool"} <= src_types, f"role source types incomplete: {src_types}"
